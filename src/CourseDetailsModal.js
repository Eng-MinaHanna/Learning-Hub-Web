...
const CourseDetailsModal = ({ course, onClose, currentUser }) => {
    ...
    // ✅ استبدال جميع عمليات axios بـ API وحذف المسار الأساسي
    ...
    const fetchVideos = () => { API.get(`/videos/${course.id}`).then(res => { setVideos(res.data); const validVideos = res.data.filter(v => canEdit || !v.video_date || new Date(v.video_date) <= new Date()); if (validVideos.length > 0 && !activeVideo) setActiveVideo(validVideos[0]); }); };
    ...
    const visibleVideos = videos.filter(vid => canEdit || !vid.video_date || new Date(vid.video_date) <= new Date());
                                        ...
                                        <div style={styles.playerContainer}>
                                            {activeVideo ? (
                                                isDriveLink(activeVideo.video_link) ? (
                                                    <iframe
                                                        src={getDriveEmbedUrl(activeVideo.video_link)}
                                                        width="100%"
                                                        height="100%"
                                                    ...
                                                    <video
                                                        src={getLocalVideoUrl(activeVideo.video_link)}
                                                        controls
                                                        onEnded={handleVideoEnd}
                                                        style={{ width: '100%', height: '100%', borderRadius: '16px', backgroundColor: 'black' }}
                                                ...
                                                {!isVideoWatched && (
                                                    <button
                                                        ...
                                                        style={{
                                                            ...
                                                            opacity: (realVideoEnded || canEdit || isDriveLink(activeVideo.video_link)) ? 1 : 0.5,
                                                            cursor: (realVideoEnded || canEdit || isDriveLink(activeVideo.video_link)) ? 'pointer' : 'not-allowed'
                                                        }}
                                                    >
                                                        {(realVideoEnded || canEdit)
                                                            ? "Mark as Completed ✅"
                                                            : (isDriveLink(activeVideo.video_link) ? "Done Watching? Click Here ✅" : "Finish video to unlock 🔒")
                                                        }
                                                    </button>
                                                )}
                                            </div>
                                        ...
                                        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(200px, 1fr))', gap: '15px', marginTop: '20px' }}>
                                            {materials.map(m => (
                                                <div key={m.id} style={styles.materialCard}>
                                                    ...
                                                    <a href={`http://localhost:5000/${m.file_path}`} target="_blank" rel="noreferrer" style={styles.downloadBtn}>Download</a>
...
const styles = {
    ...
    counterBadge: { background: 'rgba(255,255,255,0.1)', padding: '2px 8px', borderRadius: '12px', fontSize: '0.75rem', color: '#aaa' },
    ...
    optionLabel: { padding: '15px', borderRadius: '10px', cursor: 'pointer', display: 'flex', alignItems: 'center', marginBottom: '5px' },
